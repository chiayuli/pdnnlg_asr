...
if [ ! -d $working_dir/data/train ]; then
  ...
  steps/make_fbank.sh   --nj 24 $working_dir/data/train $working_dir/_log $working_dir/_fbank || exit 1;
...
if [ ! -d $working_dir/data/valid ]; then
  ...
  steps/make_fbank.sh   --nj 12 $working_dir/data/valid $working_dir/_log $working_dir/_fbank || exit 1;
...
if [ ! -d $working_dir/data/eval2000 ]; then
  ...
  steps/make_fbank.sh   --nj 12 $working_dir/data/eval2000 $working_dir/_log $working_dir/_fbank || exit 1;
...
if [ ! -f $working_dir/cnn.fine.done ]; then
    ...
    $pythonCMD pdnnlg/cmds/run_CNN_LACEA.py --train-data "$working_dir/train.pfile.*.gz,partition=2000m,random=true,stream=true" \
